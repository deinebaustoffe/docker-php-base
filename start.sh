...
# auth.json AVAILABLE.
if [[ "${AUTH_JSON}" == "1" ]]; then
    # COPY auth.json TO THE IMAGE.
    cp ${AUTH_JSON_PATH} /root/.composer/
fi
...
if [[ "${PRODUCTION}" == "1" ]]; then
    sed -i -e "s/;log_level = notice/log_level = warning/g" /etc/php7/php-fpm.d/www.conf
...
find /etc/php7/ -name "*.ini" -exec sed -i -re 's/^(\s*)#(.*)/\1;\2/g' {} \;
